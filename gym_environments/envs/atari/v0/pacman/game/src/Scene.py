...
class Scene:
        ...
        with open(settings.CHARMAP, "r") as f:
            ...
            for i in range(rows):
                ...
                for j in range(cols):
                    ...
                    if s in (".", "*"):
                        x, y = (
                            j * settings.TILE_SIZE + settings.TILE_SIZE // 2,
                            i * settings.TILE_SIZE + settings.TILE_SIZE // 2,
                        )
                        defs = DOTS[s]
            ...
            self.pacman = Pacman(
                ...
                settings.TILE_SIZE,
                speed,
                settings.TEXTURES["pacman"],
                settings.FRAMES["pacman"],
                interval,
                self,
            )
            ...
            for _ in range(num_ghosts):
                ...
                self.ghosts.append(
                    Ghost(
                        ...
                        settings.TILE_SIZE,
                        speed,
                        settings.TEXTURES["ghosts"],
                        settings.FRAMES["ghosts"][color],
                        interval,
                        self,
                        mode,
                    )
    ...
    def get_state(self):
        ...
        ip, jp = int(self.pacman.position.y // settings.TILE_SIZE), int(
            self.pacman.position.x // settings.TILE_SIZE
        )
        id, jd = self.pathfinder.find_closest_by_pred(
            (ip, jp), lambda i, j: self.map.charmap[i][j] in (".", "*")
        )
