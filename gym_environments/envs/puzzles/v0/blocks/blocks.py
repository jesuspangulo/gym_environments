...
class BlocksEnv(gym.Env):
    ...
    def __init__(self, **kwargs):
        ...
        self.observation_space = spaces.Discrete(self.n * (self.n - 1) * (self.n - 2) * self.num_directions)
    ...
    def __compute_state_result(self, d, mc, s1, s2):
        return d * self.n * (self.n - 1) * (self.n - 2) + mc * (self.n - 1) * (self.n - 2) + s1 * (self.n - 2) + s2
